...
class MyRequestsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String statusLabel(String status) {
      switch (status) {
        case 'ONGOING': return t.translate('ongoing');
        case 'COMPLETED': return t.translate('completed');
        case 'CANCELLED': return t.translate('cancelled');
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Column(children: [
        Container(color: theme.colors.background.card, padding: const EdgeInsets.all(16), child: Align(alignment: AlignmentDirectional.centerStart, child: Text(t.translate('myRequests'), style: theme.textStyle.titleXLarge.copyWith(color: theme.colors.shade.primary)))),
        Expanded(child: requests.isEmpty
          ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.inbox_outlined, size: 64, color: theme.colors.shade.tertiary), const SizedBox(height: 16), Text(t.translate('noRequestsYet'), style: theme.textStyle.titleSmall.copyWith(color: theme.colors.shade.secondary))]))
          : ListView.separated(
                ...
                return Container(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Row(children: [
                      ...
                      Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: statusBg, borderRadius: BorderRadius.circular(theme.radius.full)), child: Text(statusLabel(r.status), style: theme.textStyle.labelMediumSemibold.copyWith(color: statusColor))),
                    ]),
                    ...
                    Row(children: [
                      ...
                      const Spacer(),
                      Text('${r.offers} ${t.translate('offers')}', style: theme.textStyle.bodySmallMedium.copyWith(color: theme.colors.brand.primary)),
                    ]),
                  ]),
                );
              },
